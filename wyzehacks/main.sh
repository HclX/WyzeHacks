...
on_reboot() {
    ...
    cp /tmp/boot.log $WYZEHACKS_DIR/reboot_$TS.log
    sync
    umount -f $WYZEHACKS_DIR
...
check_config() {
    ...
    cp /tmp/tmp_config $WYZEHACKS_CFG
...
check_update() {
    ...
    if ! cp $LATEST_UPDATE/wyzehacks/wyzehacks.bin $WYZEHACKS_BIN; then
        echo "WyzeHack: Copying $LATEST_UPDATE/wyzehacks/wyzehacks.bin to $WYZEHACKS_BIN failed."
        return 0
    fi
...
cmd_run() {
    ...
    echo "WyzeHack: WyzeHack version: $WYZEHACKS_VER"
    ...
    if [ -z "$NFS_ROOT" ]; then
        ...
        if mount_nfs; then
            ...
            cp $WYZEHACKS_CFG /media/mmcblk0p1/wyzehacks/wyzehacks.cfg
